...
const stoneDropWitnesses = {
  get_deposit_coin: (ctx: any) => [ctx.privateState, { nonce: new Uint8Array(32), color: new Uint8Array(32), value: 0n }] as const,
  get_claim_coin: (ctx: any) => [ctx.privateState, { nonce: new Uint8Array(32), color: new Uint8Array(32), value: 0n, mt_index: 0n }] as const,
  get_claim_recipient: (ctx: any) => [ctx.privateState, { bytes: new Uint8Array(32) }] as const,
  get_claim_value: (ctx: any) => [ctx.privateState, 0n] as const,
};

async function main() {
  console.log(`\n=== Deploy StoneDrop to ${NETWORK_LABEL} ===\n`);

  const { compiledContract, zkConfigPath } = await loadCompiledContractWithWitnesses(CONTRACT_NAME, stoneDropWitnesses);
